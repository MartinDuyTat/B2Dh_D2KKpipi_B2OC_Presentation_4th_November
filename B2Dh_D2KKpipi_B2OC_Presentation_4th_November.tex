...
\begin{frame}{Binned measurement of $\gamma$}
  \begin{itemize}
    \item{Enhance sensitivity through binning of phase space}
  ...
  \end{itemize}
  \begin{figure}
    \includegraphics[width = 0.50\textwidth]{Plots/KsPiPi_optimal.png}
  \end{figure}
\end{frame}
...
\begin{frame}{The $D\to K^+K^-\pi^+\pi^-$ decay}
  \vspace{0.0cm}
  {\Large Measurement of $\gamma$ from $B^\pm\to DK^\pm$, $D\to K^+K^-\pi^+\pi^-$}
  \vspace{0.5cm}
  \begin{itemize}
    ...
    \end{itemize}
    \item{State of the art amplitude analysis by LHCb LHCb-PAPER-2018-041:}
    \begin{itemize}
      \item{Used to develop efficient binning scheme in this analysis}
...
\begin{frame}{Binning scheme requirements}
  ...
  \begin{itemize}
    \item{Avoid dilution of strong phases when integrating over bins}
    \item{Enhance interferences between $B^\pm\to D^0h^\pm$ and $B^\pm\to\bar{D^0}h^\pm$ amplitudes}
  \end{itemize}
  \vspace{0.4cm}
...
\begin{frame}{Study of $\gamma$ precision}
  ...
  \end{figure}
  \begin{center}
    Precision of $\gamma$ in unbinned fit: $12^\circ$ \\
...
\begin{frame}{Samples}
  \begin{itemize}
    ...
    \end{itemize}
    \item{Data samples: 2011-2018 (2011-2012 not processed yet)}
...
\begin{frame}{$c_i$ and $s_i$ systematic uncertainty}
  \begin{itemize}
    ...
    \item{Originate from (mostly statistical) uncertainty of $c_i$ and $s_i$ in BESIII analysis}
    \item{By far the largest systematic uncertainty}
...
\begin{frame}{Remaining systematic uncertainties}
  \begin{itemize}
    \setlength\itemsep{1em}
    \item{For these systematics, generate toy datasets, fit with default model and take the bias as a systematic:}
    \begin{itemize}
      \item{Small backgrounds ($D\to K(X)l\nu_l$, $D\to K\pi\pi\pi$, $\Lambda_b$)}
      \item{Bin dependent mass shape}
      \item{Low mass physics effects}
    \end{itemize}
    \item{For these systematics, do multiple fits to data while smearing parameters:}
    \begin{itemize}
      \item{Mass shape}
      \item{Fixed yield fractions}
      \item{PID efficiency}
    \end{itemize}
    \item{Fit bias: From toys}
  \end{itemize}
\end{frame}

\begin{frame}{Efficiency difference between $B^\pm\to DK^\pm$ and $B^\pm\to D\pi^\pi$}
  \begin{figure}
    \centering
...
\begin{frame}{Interpretation in terms of $\gamma$}
  ...
  \end{align*}
  \begin{itemize}
    \item{Note: Currently CP observables are determined using $c_i$ and $s_i$ from the \underline{LHCb model}}
